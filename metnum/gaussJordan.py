...
def backwardElimination(A, N, urutanPivot):
    for i in range(urutanPivot-1, -1, -1):
        # add to row i multiple k times row urutanPivot
        # A.add_multiple_of_row(i, urutanPivot, k)
        multipler = -A[i][urutanPivot] / A[urutanPivot][urutanPivot]
        A.add_multiple_of_row(i, urutanPivot, multipler)
...
def normalizeRows(A, N):
    for i in range(N):
        scaledNumber = 1 / A[i, i]
        A.rescale_row(i, scaledNumber)
    return A

def gaussJordanElimination(A, N):
    for i in range(N-1):
        A = partialPivoting(A, N, i)
        A = forwardElimination(A, N, i)
        
    for i in range(N-1, 0, -1):
        A = backwardElimination(A, N, i)
